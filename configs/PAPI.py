...
model = dict(
    ...
    decode_head=dict(
        ...
        num_classes=1,  # Number of segmentation classs. Usually 19 for cityscapes, 21 for VOC, 150 for ADE20k.
...
data_root = '/media/mingfan/DataHDD/DATA_PAIP/Segmentation/'  # Root path of data.
...
train_pipeline = [  # Training pipeline.
    ...
    dict(type='RandomCrop',  # Augmentation pipeline that randomly crop a patch from current image.
        crop_size=(224, 224),  # The crop size of patch.
    ...
    dict(type='Collect',  # Pipeline that decides which keys in the data should be passed to the segmentor
        keys=['patchData', 'patchLabel'])
]
test_pipeline = [
    dict(type='LoadImageFromFile'),  # First pipeline to load images from file path
    dict(
        type='MultiScaleFlipAug',  # An encapsulation that encapsulates the test time augmentations
        img_scale=(256, 256),  # Decides the largest scale for testing, used for the Resize pipeline
...
data = dict(
    samples_per_gpu=20,  # Batch size of a single GPU
    workers_per_gpu=2,  # Worker to pre-fetch data for each single GPU
    train=dict(  # Train dataset config
        type='CityscapesDataset',  # Type of dataset, refer to mmseg/datasets/ for details.
        data_root='/media/mingfan/DataHDD/DATA_PAIP/Segmentation/',  # The root of dataset.
        img_dir='img/train',  # The image directory of dataset.
        ann_dir='label/train',  # The annotation directory of dataset.
    ...
    val=dict(  # Validation dataset config
        ...
        data_root='/media/mingfan/DataHDD/DATA_PAIP/Segmentation/',  # The root of dataset.
        img_dir='img/val',  # The image directory of dataset.
        ann_dir='label/val',  # The annotation directory of dataset.
